...
func (p *Pinger) logStats(stats *Stats) {
	...
	if stats.Error != nil {
		_, _ = fmt.Fprintf(p.out, "%s: Ping %s(%s) %s(%s) - time=%s dns=%s", timestampFmt, p.url.String(), stats.Address, status, p.formatError(stats.Error), stats.Duration, stats.DNSDuration)
	} else {
